...
[![CI](https://github.com/palak-38/job_digest/actions/workflows/ci.yml/badge.svg)](https://github.com/palak-38/job_digest/actions/workflows/ci.yml)
[![Daily digest](https://github.com/palak-38/job_digest/actions/workflows/daily-digest.yml/badge.svg)](https://github.com/palak-38/job_digest/actions/workflows/daily-digest.yml)
...
│    → score (small model, ONE batched call): rates the job against     │
│      EVERY role rubric at once (0-10 + reason + fresher-experience    │
│      check); best score gates                                        │
│    → tailor (large model, matched jobs only): returns a validated     │
│      JSON edit set over the structured resume — summary + ≤6 bullet   │
│      edits, never invented facts, never a full rewrite                │
...
- **Two-tier models, forced by the free tier's rate limit.** Groq's free tier caps each model at 8k tokens/min, *per model*. Scoring every job against 3 rubrics as 3 separate calls — on the same model used for tailoring — blew that budget and 429'd every run. Fix: scoring (a 0-10 classification) runs on a small fast model, batched into **one** call covering all role rubrics at once (~3x fewer tokens); tailoring (where writing quality matters) stays on the larger model and only runs on jobs that clear the gate. Two models, two separate budgets, plus backoff/retry on any remaining 429 — the pipeline now runs comfortably inside the free tier by design, not by paying for headroom.
...
python -m pytest app/tests  # 81 tests, no network required
```
...
| `GROQ_SCORING_MODEL` / `GROQ_TAILORING_MODEL` | model per phase (default `llama-3.1-8b-instant` / `openai/gpt-oss-120b`) — separate free-tier rate-limit budgets | no |
| `SCORE_THRESHOLD`, `DEFAULT_ROLE`, `JOBS_PER_RUN`, `CANDIDATE_EXPERIENCE`, `ROLE_RESUME_DOC_IDS` | tuning, all defaulted | no |
...
- Scoring and tailoring run sequentially across jobs, on purpose: parallel bursts are exactly what trips the free tier's per-minute token limit. No concurrency claims until that's built and deliberately budgeted for.
- Scoring quality is bounded by the rubric text and the model; the threshold (default 6) is a tuning knob, not a guarantee. The scoring model is small and fast by design (see above) — a paid tier would allow a stronger model here without the rate-limit tradeoff.
